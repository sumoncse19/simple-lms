...
import { getCourse, getEnrollment, updateEnrollmentProgress } from '@/services/storage'
...
const Learning = () => {
  ...
  if (!course || !enrollment) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Alert>
          <AlertDescription>Course not found or you are not enrolled</AlertDescription>
        </Alert>
      </div>
    )
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4">
            <Button
              onClick={() => handleProgressUpdate(Math.min(enrollment.progress + 10, 100))}
            ...
            {enrollment.progress > 0 && (
              <Button
                variant="outline"
                onClick={() => handleProgressUpdate(Math.max(enrollment.progress - 10, 0))}
                disabled={enrollment.status === 'completed'}
              >
